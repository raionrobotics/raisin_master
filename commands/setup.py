...
def _repo_version_from_installed(
    ...
    if not repo_dir.is_dir():
        return None
    # Prefer a platform-matching release.yaml if present; otherwise pick any.
    preferred = (
        repo_dir
        / g.os_type
        / g.os_version
    ...
    if preferred.is_file():
        try:
            details = yaml.safe_load(preferred.read_text(encoding="utf-8", errors="ignore"))
...
def _repo_version(src_dir: Path, release_install_dir: Path, repo_name: str) -> Optional[str]:
...
def _update_release_yaml_dependencies_in_place(
    ...
    # 1) Inline list form: dependencies: [a, b]
        ...
        if re.match(r"^\s*dependencies:\s*$", line):
            ...
            while j < len(lines):
                if re.match(rf"^{re.escape(base_indent)}\S", lines[j]):
                    break
                j += 1
            insert = "".join([ensure_trailing_newline(f"{item_indent}- {s}") for s in add_specs])
            lines[j:j] = [insert]
            release_yaml.write_text("".join(lines), encoding="utf-8")
            return

    # 3) No dependencies key: insert after version if possible, else append.
...
def guard_src_repo_release_yaml_dependencies(
    ...
    if conflicts:
        # Best-effort message; duplicates across repos make dependency inference ambiguous.
        print("❌ Error: Duplicate package directories found across repos for referenced packages.")
    ...
    for pkg, sources in package_sources.items():
        # Safe because conflicts above exited.
        package_to_repo[pkg] = sources[0][0]

    # Add binary packages discovered in release/install
    ...
    for pkg_name, (repo_name, evidence_path) in binary_pkg_to_repo.items():
        ...
        # If already mapped to same repo, keep existing mapping.
